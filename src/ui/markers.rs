...
pub(super) fn draw(state: &UIState, ui: &Ui, marker: &Marker, color: [f32; 3], draw_list: DrawList) {
	...
	if let Some(meters) = marker.meters {
		...
		if let Some(minimap_viewport) = state.vision.minimap_bounds {
			if let Some(heightmap) = squadex::heightmaps::get_current() {
				...
				let p0_x = (p0_xf * heightmap.width as f64).round() as i32;
				let p0_y = (p0_yf * heightmap.height as f64).round() as i32;

				let p1_x = (p1_xf * heightmap.width as f64).round() as i32;
				let p1_y = (p1_yf * heightmap.height as f64).round() as i32;
